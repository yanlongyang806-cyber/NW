...
static bool 
IsCharacterAllegianceUnlocked(NOCONST(Entity) *playerEnt, GameAccountData *gameAccountData, ContainerID virtualShardID, AllegianceDef *allegianceDef)
{
    ...
    if ( virtualShardID )
    {
        ...
        {
            if (allegianceDef->bCannotPlayUGC)
            {
                return false;
            }
    ...
    // If this allegiance does not need an unlock, or the player is AL9, then it's unlocked
    if ( SAFE_MEMBER2(playerEnt, pPlayer, accessLevel) >= ACCESS_DEBUG ||
         SAFE_MEMBER2(playerEnt, pPlayer, accountAccessLevel) >= ACCESS_DEBUG ||
         !allegianceDef || !allegianceDef->pcRequiredPermission )
    {
        return true;
    }

    return GamePermission_GADHasToken(gameAccountData, allegianceDef->pcRequiredPermission);
}
...
aslLogin2_InitializeNewCharacter(Login2State *loginState, NOCONST(Entity) *playerEnt)
{
    ...
    else
    {
        ...
        if ( characterCreationData->allegianceName && characterCreationData->allegianceName[0] )
        {
            ...
            else if ( !IsCharacterAllegianceUnlocked(playerEnt, gameAccountData, characterCreationData->virtualShardID, allegianceDef) )
            {
                entity_DestroyLocalGameAccountDataExtract(&pExtract);
            ...
            else if ( allegianceDef->bCannotPlayUGC && loginState->UGCCharacterPlayOnly )
            {
                entity_DestroyLocalGameAccountDataExtract(&pExtract);
                NEWCHARFAIL("Allegiance is not valid for UGC");
                PERFINFO_AUTO_STOP_FUNC();
                return false;               
            }
        }
        ...
        if ( allegianceDef && speciesDef && eaSize(&allegianceDef->eaStartSpecies) )
        {
            for ( i = eaSize(&allegianceDef->eaStartSpecies)-1; i >= 0; --i )
            {
                if ( GET_REF(allegianceDef->eaStartSpecies[i]->hSpecies) == speciesDef )
                {
                    ...
                    {
						if(Species_IsUnlocked(speciesDef, gameAccountData))
                        {
                            break;
                        }
                    }
		...
		//Initialize UI version
		playerEnt->pPlayer->pUI->iKeybindsVersion = KEYBIND_SETTINGS_VERSION;
...
{
    ExprFuncTable* stFuncTable = exprContextCreateFunctionTable("AslLoginServerLoadNewCharacterValidation");
    S32 i;
