...
AUTO_STRUCT;
typedef struct AttribStatsPresetOverrides
{
	int iTotalPoints;					AST(STRUCTPARAM)
	AssignedStats** ppAttribStats;		AST(NAME(AttribStats))
}AttribStatsPresetOverrides;
...
typedef struct AttribStatsPresetDef
{
	...
	AttribStatsPresetOverrides** ppOverrides; AST(NAME(Override))
...
typedef struct EntCreateParams
{
	...
	Expression *pExprFacing;			AST(NAME(ExprBlockFacing, Facing), LATEBIND)
	...
	U32 bUseOwnersFaction : 1;
		// this sets the created entity's faction to the owner's faction

	U32 bAlwaysRerollCrits : 1;
		// This causes all powers used by the created ent to reroll its crit flag.
...
typedef struct ShieldParams
{
	...
	bool bIgnoreShieldSensitivity;
		// if set, this shield will ignore all shield sensitivity
