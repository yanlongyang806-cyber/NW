...
//
    ...
    required.strings.forEach(function (r) {
        var x = r.match(/^(\.\.?)?\//) ? path.resolve(dir, r) : r; // CRYPTIC fixed resolve(dir, r) instead of resolve(opts.root, r)
    ...
    this.append((self.debug ? wrappers.entry_debug : wrappers.entry)
        .replace(/\$__filename/g, function () {
            var f = path.basename(target); // CRYPTIC
            return JSON.stringify(f) // CRYPTIC
        })
        .replace(/\$body/, function () {
            return self.debug
                ? JSON.stringify(body + '\n//@ sourceURL=' + pruneCrypticBasePath(target)) // CRYPTIC added prune
                : body
            ;
        })
    );
...
// CRYPTIC added
function pruneCrypticBasePath(name)
{
    var re = /.*\\gateway\\build\\(tmp|deploy)\\(debug|prod)\\(.*)/i;
    var res = re.exec(name);
    if(res && res[3])
    {
        name = res[3].replace(/node_modules\\cryptic/, 'cryptic');
        return name;
    }
    else
        return name;
...
Wrap.prototype.require = function (mfile, opts) {
    ...
    || self.has(mfile, '/node_modules/' + mfile + '/package.json')) {
        ...
        var file;
        try {
            file = self.resolver(mfile, opts.basedir);
        }
        catch (err) {
            if (/^\//.test(mfile)) {
                try {
                    file = self.resolver(
                        path.relative(opts.basedir, mfile),
                        opts.basedir
                    );
                }
                catch (e) {}
            ...
            if (!file) throw new Error(
                'Cannot find module ' + JSON.stringify(mfile)
