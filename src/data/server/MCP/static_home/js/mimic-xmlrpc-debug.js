...
var result = xhr.responseXML;if (xhr.responseText!=""&&""+result==""&&window.ActiveXObject){var t=document.createElement('xml');t.setAttribute('innerHTML',xhr.responseText);t.setAttribute('id','_MSIE_Sucks_Donkey_Balls');document.body.appendChild(t);result=document.getElementById('_MSIE_Sucks_Donkey_Balls');document.body.removeChild(document.getElementById('_MSIE_Sucks_Donkey_Balls'));}return new XmlRpcResponse(result);
};
      ...
      switch(XmlRpc.getDataTag(this.params[this.params.length - 1])) {	   
	    case "dateTime.iso8601":
 	    ...
 	    case "double":		
		  ...
		  break;
 	    case "string":
			//alert("overwriting:" + this.params[this.params.length - 1]);
